...
from src.schema import DataSchema, SchemaValidator, SheetSchema, FieldSpec


class TestDictValidation:
    def test_required_field_present_passes(self):
        schema = DataSchema(sheets={
            "date.全局": SheetSchema(fields={"name": FieldSpec(required=True)})
        })
        validator = SchemaValidator(schema)
        errors = validator.validate({"date.全局": {"name": "test"}})
        assert len(errors) == 0
    ...
    def test_required_field_missing_reports_error(self):
        ...
        validator = SchemaValidator(schema)
        errors = validator.validate({"date.全局": {"other": "val"}})
        assert len(errors) == 1
        assert "date.全局.name" in errors[0]

    def test_type_check_string(self):
        schema = DataSchema(sheets={
            "date.全局": SheetSchema(fields={"name": FieldSpec(field_type="str")})
        })
        validator = SchemaValidator(schema)
        errors = validator.validate({"date.全局": {"name": 123}})
        assert len(errors) == 1
...
class TestDataFrameValidation:
    def test_dataframe_required_column_present_passes(self):
        ...
        validator = SchemaValidator(schema)
        errors = validator.validate({"form.商品": df})
        assert len(errors) == 0
    ...
    def test_dataframe_column_missing_reports_error(self):
        ...
        errors = validator.validate({"form.商品": df})
        assert len(errors) == 1
        assert "form.商品" in errors[0]

    def test_dataframe_rows_present_passes(self):
        schema = DataSchema(sheets={
            "form.商品": SheetSchema(required=True)
        })
...
class TestDataSchemaConstruction:
    def test_from_dict_simple(self):
        d = {"sheets": {"date.全局": {"required": True, "fields": {"name": True}}}}
        schema = DataSchema.from_dict(d)
        assert schema.sheets["date.全局"].required is True
        assert schema.sheets["date.全局"].fields["name"].required is True
