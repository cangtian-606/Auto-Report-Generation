...
def convert():
    ...
    for p in doc.paragraphs:
        ...
        replace_para_text(p, '大华咨字[2026]XXXX号', '{{ 全局.报告文号 }}')
        replace_para_text(p, '二〇二六年五月【】日', '二〇二六年五月{{ 全局.报告日期_日 }}')

    # ============ TABLE[0]: Cover ============
    t0 = doc.tables[0]
    replace_merge_cell(t0.cell(0, 0), '【】', '{{ 全局.项目公司简称 }}')
    replace_merge_cell(t0.cell(2, 0), '大华咨字[2026]XXXX号', '{{ 全局.报告文号 }}')

    # ============ TABLE[1]: Company name mapping ============
    t1 = doc.tables[1]
    for ri in range(1, len(t1.rows)):
        replace_merge_cell(t1.cell(ri, 0), 'xxxx', '{{ 全局.项目公司全称 }}')
        replace_merge_cell(t1.cell(ri, 1), 'xxxx', '{{ 全局.项目公司简称 }}')
    ...
    t6 = doc.tables[6]
    replace_merge_cell(t6.cell(0, 2), '【】', '{{ 全局.项目公司简称 }}')
    ...
    replace_merge_cell(t7.cell(1, 3), 'xxxx', '{{ PC合同.设备费 | money }}')
    replace_merge_cell(t7.cell(2, 3), 'xxxx', '{{ PC合同.建安费 | money }}')
    replace_merge_cell(t7.cell(3, 3), 'xxxx', '{{ PC合同.其他费 | money }}')
    replace_merge_cell(t7.cell(4, 3), 'xxxx', '{{ PC合同.合同总价 | money }}')
    ...
    t9 = doc.tables[9]
    replace_merge_cell(t9.cell(0, 2), '【】项目', '{{ 全局.项目公司简称 }}项目')
    fa_fields = {
    ...
    for ri in range(1, min(len(t10.rows), len(period_names) + 1)):
        ...
        set_cell_text(t10, ri, 1, '{{ 发电量.' + pn + '.自发自用 | num }}')
        set_cell_text(t10, ri, 2, '{{ 发电量.' + pn + '.余电上网 | num }}')
        set_cell_text(t10, ri, 3, '{{ 发电量.' + pn + '.发电量小计 | num }}')
        set_cell_text(t10, ri, 4, '{{ 发电量.' + pn + '.自发自用占比 | percent }}')
    ...
    for ri in range(1, len(t12.rows)):
        ...
        if field:
            ...
            for ci in range(1, min(len(t12.columns), len(period_keys) + 1)):
                set_cell_text(t12, ri, ci, '{{ 资产负债表.' + safe + '_' + period_keys[ci - 1] + ' | money }}')
    ...
    for ri in range(1, len(t13.rows)):
        ...
        if field:
            ...
            for ci in range(1, min(len(t13.columns), len(pl_periods) + 1)):
                set_cell_text(t13, ri, ci, '{{ 利润表.' + safe + '_' + pl_periods[ci - 1].replace('-', '') + ' | money }}')
    ...
    if len(t14.rows) > 1:
        ...
        r1 = t14.add_row()
        set_cell_text(t14, 1, 0, '{%tr for i in 关联方 %}')
    ...
    if len(t15.rows) > 1:
        ...
        r1 = t15.add_row()
        set_cell_text(t15, 1, 0, '{%tr for i in 关联方余额 %}')
    ...
    if len(t16.rows) > 1:
        ...
        r1 = t16.add_row()
        set_cell_text(t16, 1, 0, '{%tr for i in 关联方交易 %}')
