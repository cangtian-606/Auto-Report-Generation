...
def _process_paragraphs(doc):
    for p in doc.paragraphs:
        ...
        if '大华咨字' in text:
            _replace_runs(p, {'大华咨字[2026]XXXX号': '{{ date.全局.报告文号 }}'})

        if '系由【】出资组建' in text:
            _replace_runs(p, {
                '【】': '{{ date.历史沿革.出资方 }}',
                '【】年【】月【】日': '{{ date.历史沿革.设立日期 }}',
                '【】核发的': '{{ date.历史沿革.核发机关 }}',
                '统一社会信用代码为【】': '统一社会信用代码为{{ date.基本情况.信用代码 }}'
            })

        if '【】项目位于【】' in text:
            _replace_runs(p, {
                '【】项目位于【】，': '{{ date.项目概况.项目简称 }}项目位于{{ date.项目概况.项目地址 }}，',
                '实际装机容量为【】': '实际装机容量为{{ date.项目概况.装机容量 | num }}',
            })

        if '不含税工程总价为【】' in text:
            _replace_runs(p, {
                '【】项目竣工安装容量为【】，': '{{ date.项目概况.项目简称 }}项目竣工安装容量为{{ date.项目概况.竣工容量 | num }}，',
                '不含税工程总价为【】元': '不含税工程总价为{{ date.项目概况.工程总价 | money }}元',
                '不含税建设单价为【】': '不含税建设单价为{{ date.项目概况.建设单价 | money }}',
            })

        if '合同总价为' in text and '分项价格如下' in text:
            _replace_runs(p, {
                '【】项目合同总价为': '{{ date.项目概况.项目简称 }}项目合同总价为',
                '2,728,160.00': '{{ date.PC合同.合同总价 | money }}',
            })

        if '乙方（承包单位）：【】' in text:
            _replace_runs(p, {'【】': '{{ date.PC合同.承包单位 }}'})

        if '工程内容及规模：【】' in text:
            _replace_runs(p, {'【】': '{{ date.PC合同.工程内容 }}'})

        if '电费结算单价的【】%' in text or '分时电度用电价格的【】%' in text:
            _replace_runs(p, {'【】': '{{ date.EMC合同.结算折扣率 }}'})

        if text.startswith('②用能方：') and '【】' in text:
            _replace_runs(p, {'【】': '{{ date.EMC合同.用能方 }}'})

        if '自发自用电量占总发电量比例为' in text:
            _replace_runs(p, {'*': '{{ date.运营情况.自发自用占比 | percent }}'})

        if '上网电费含税单价为' in text:
            _replace_runs(p, {
                '*元/kWh': '{{ date.运营情况.上网电价 | money }}元/kWh',
                '*65%': '{{ date.运营情况.结算折扣率 }}%'
            })

        if '20' in text and 'x年' in text and '电费未回款' in text:
            text2 = text
            text2 = text2.replace('202x年x月至x月', '{{ date.运营情况.自发自用未回款期间 }}')
            text2 = text2.replace('202x年x月', '{{ date.运营情况.余电上网未回款期间 }}')
            for run in p.runs:
                run.text = ''
            if p.runs:
                p.runs[0].text = text2
        ...
        if '二〇二六年五月【】日' in text:
            _replace_runs(p, {'【】': '{{ date.全局.报告日期_日 }}'})


def _process_tables(doc):
    # ===== Table[0]: Cover page =====
    t0 = doc.tables[0]
    _cell_replace(t0, 0, 0, '【】', '{{ date.全局.项目公司简称 }}')
    _cell_replace(t0, 2, 0, '大华咨字[2026]XXXX号', '{{ date.全局.报告文号 }}')

    # ===== Table[1]: Company full/short name mapping =====
    t1 = doc.tables[1]
    for ri in range(1, len(t1.rows)):
        _cell_replace_all(t1, ri, 0, 'xxxx', '{{ date.全局.项目公司全称 }}')
        _cell_replace_all(t1, ri, 1, 'xxxx', '{{ date.全局.项目公司简称 }}')
    ...
    for ri in range(len(t2.rows)):
        ...
        if 'xxxx' in cell_text:
            key = t2.cell(ri, 0).text.strip()
            var = field_map_t2.get(key, '{{ date.基本情况.' + key + ' }}')
            _cell_replace_all(t2, ri, 1, 'xxxx', var)

    # ===== Table[3]: Shareholder table → loop =====
    t3 = doc.tables[3]
    _make_shareholder_loop(t3, 'form.股东出资')
    ...
    t5 = doc.tables[5]
    _make_shareholder_loop(t5, 'form.股东出资_历史')

    # ===== Table[6]: Project approvals =====
    t6 = doc.tables[6]
    for ri in range(len(t6.rows)):
        _cell_replace_all(t6, ri, 0, '【】', '{{ date.全局.项目公司简称 }}')
    ...
    t9 = doc.tables[9]
    _cell_replace_all(t9, 0, 2, '【】项目', '{{ date.全局.项目公司简称 }}项目')
    field_vals = {
    ...
    t14 = doc.tables[14]
    if len(t14.rows) > 1:
        _make_cell_for_loop(t14, 0, 'form.关联方')

    # ===== Table[15]: Related party balances → loop =====
    t15 = doc.tables[15]
    if len(t15.rows) > 1:
        _make_cell_for_loop(t15, 0, 'form.关联方余额')

    # ===== Table[16]: Related party transactions → loop =====
    t16 = doc.tables[16]
    if len(t16.rows) > 1:
        _make_cell_for_loop(t16, 0, 'form.关联方交易')
